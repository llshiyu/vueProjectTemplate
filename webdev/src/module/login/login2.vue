        ...
        <div class="login-box">
            <el-form label-position="right" :model="formLogin" :rules="loginRules" ref="loginForm">
                ...
                <el-form-item  prop="phone">
                    <el-input maxlength="11" v-model.trim="formLogin.phone" placeholder="请输入手机号"></el-input>
                </el-form-item>
                <el-form-item prop="phoneCode" class="clearfix">
                    <el-input maxlength="14" class="fl code-input" v-model.trim="formLogin.phoneCode" placeholder="请输入手机验证码"></el-input>
                    <el-button type="primary" class="fl code-btn" :class="noClickBtn?'no-click':''" @click="getPhoneCode()">{{phoneCodeText}}{{noClickBtn?'('+countdownNum+')':''}}</el-button>
                </el-form-item>
...
<script>
    ...
    export default {
        data() {
            ...
            var regularPhone = /^1[345678][0-9]{9}$/; // 手机号验证规则
            ...
            var validatePhone = (rule, value, callback) => {
                if (value === '') {
                    ...
                    if(!regularPhone.test(value) || value.length !== 11) {
                        callback(new Error('手机号格式有误，请核对后重新输入'));
                    }else{
                        callback();
                    }
                }
            ...
            return {
                ...
                codeImg: '',
                codeKey:'',
                noClickBtn: false, // 不可点击样式
                countdownNum: 60, // 60秒后再次获取
                setInterval:null, // 60秒倒计时
                phoneCodeText:'获取短信验证码', // 重新发送
            }
        },
        mounted() {
            this.init();
        ...
        methods: {
            ...
            getPhoneCode(){
                this.phoneCodeText = '重新发送';
                if(!this.noClickBtn){
                    let param = {
                        phone: this.formLogin.phone
                    };
                    // this.$api.common.getPhoneCode(param).then(res => {
                    //     if (res.meta.code == 0) {
            ...
            loginAjax() {
                let param = {
                    ...
                    phone: this.formLogin.phone,
                    phoneCode: this.formLogin.phoneCode,
