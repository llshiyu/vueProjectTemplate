<template>
    <div class="detail-pack" >
        ...
        <el-breadcrumb separator="/" class="order-breadcrumb">
            <el-breadcrumb-item :to="{ path: '/canteenPurchase' }">采购货源</el-breadcrumb-item>
            <el-breadcrumb-item>商品详情 </el-breadcrumb-item>
        </el-breadcrumb>

        <div v-show="havePageCommodityData">
                    ...
                    <div class="total-details-pack" v-if="getGoodsList && getGoodsList.length>0">
                        ...
                        <div class="list-pack-height">
                            <div class="list-pack" v-for="obj in getGoodsList">
                                <p class="table-1"><span class="details-icon" :style="{  'background-image': obj.mainPic ?'url('+obj.mainPic+')':'' }"></span><span class="details-text cursor-hand" :title="obj.goodsName" @click="goCanteenDetails_2(obj)">{{  obj.goodsName,17 | textEllipsis  }}</span></p>
                                <p class="table-2 red"><span>¥</span>{{ obj.unitPrice && obj.unitPrice.toFixed(2) }}×{{ obj.goodsNum }}</p>
                                <p class="table-3 padding-right5"><i class="delete-icon cursor-hand" @click="cartRemove(obj)"></i></p>
                            </div>
                        </div>
                        <p class="title bottom"><span class="table-1"><i class="title-text">合计</i><i class="red"><span>¥</span>{{ totalMoney }}</i></span><span class="go-see"><i class="cursor-hand" @click="goCanteenCart()">去查看</i></span></p>
                    </div>
            ...
            <div class="details-nav-pack">
                <div class="left-pack">
                    <div class="left-img" :style="{  'background-image': imgData.showImg && imgData.showImg.data && imgData.showImg.data.url?'url('+imgData.showImg.data.url+')':'' }"></div>
                ...
                <div class="right-pack">
                    <div class="details-information">
                        <p class="information-title">供应商介绍</p>
                        <div class="information-content-pack">
                            <p class="content-text">供应商名称：{{ supplierData.supplierName }}</p>
                            <p class="content-text">供应商地址：{{ supplierData.defaultAddressName }}</p>
                ...
                <div class="middle-pack">
                    ...
                    <div class="middle-foot">
                        ...
                        <p class="p"><span class="title">库存量</span><span>{{ pageCommodityData.stock }}</span></p>
                        <p class="p"><span class="title">数量</span><span
                                class="add-subtract-pack cursor-hand"><i @click="addNum(-1)"></i><i><input
                                type="number" step="0.01" v-model="addCommodityNum" placeholder="0" @input="addNum()"></i><i class="cursor-hand" @click="addNum(1)"></i></span>&nbsp;/&nbsp;件</p>
                    </div>
                    <div class="middle-foot-button">
                        <p class="button cursor-hand"  @click="addShopping()">加入订货单</p>
                    </div>
                </div>
            </div>
            ...
            <div class="explain-pack">
                ...
                <div class="explain-content-pack">
                    <!--   <p class="content-text">马铃薯原产于南美洲安第斯山区，人工栽培史最早可追溯到公元前8000年到5000年的秘鲁南部地区。安第斯山脉海拔3800米之上的的的喀喀湖区可能是最早栽培马铃薯的地方。在距今大约7000年前，一支印第安部落由东部迁徙到高寒的安第斯山脉，在的的喀喀湖区附近安营扎寨，以狩猎和采集为生，是他们最早发现并食用了野生的马铃薯。</p>
                       <div class="img-list-pack">
                           <img :src="Img" v-for="i in 5">
                       </div>
                       <p class="content-text">马铃薯原产于南美洲安第斯山区，人工栽培史最早可追溯到公元前8000年到5000年的秘鲁南部地区。安第斯山脉海拔3800米之上的的的喀喀湖区可能是最早栽培马铃薯的地方。在距今大约7000年前，一支印第安部落由东部迁徙到高寒的安第斯山脉，在的的喀喀湖区附近安营扎寨，以狩猎和采集为生，是他们最早发现并食用了野生的马铃薯。</p>
       -->
...
<script>
    import js from "./canteenDetails.js"
    export default js
</script>
